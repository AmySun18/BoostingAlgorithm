...
public class Coverage extends Applet implements ActionListener, ItemListener, ChangeListener
{
    ...
    //low detection boost
//    JCheckBox neighborBoostCheckbox = new JCheckBox("selfBoost I");
//    JCheckBox selfBoostCheckbox = new JCheckBox("selfBoost II"); 
//    JCheckBox concaveBoostCheckbox = new JCheckBox("concaveBoost");
//  JRadioButton boost1Radio = new JRadioButton("Boost");
  
//  JRadioButton selfBoost2Radio= new JRadioButton("selfBoost II"); 
  JRadioButton powerBoostRadio = new JRadioButton("PBoost");
  JRadioButton neighborBoostRadio = new JRadioButton("neighborBoost");
  JRadioButton phiBoostRadio = new JRadioButton("phiBoost");
  JRadioButton noBoostRadio = new JRadioButton("noBoost");
//  JRadioButton powerneighborBoostRadio = new JRadioButton("PowNei");
//  JRadioButton phineighborBoostRadio = new JRadioButton("PhiNei");
  JRadioButton randomBoostRadio = new JRadioButton("RandomBoost");
  JRadioButton simlatedAnnealingBoostRadio = new JRadioButton("simABoost");

  
 
  JCheckBox allnode = new JCheckBox("All");
  
  ArrayList<JCheckBox> nodestatus = new ArrayList<JCheckBox>();
  
  //boosting duration and boosting function parameters
  JTextField boost_duration_textfield = new JTextField();
  JTextField boost_iteration_textfield = new JTextField();
  JTextField boost_neighborgain_textfield = new JTextField();
  JTextField boost_neighborpower_textfield = new JTextField();
  JTextField boost_Power_textfield = new JTextField();
  JTextField boost_PowerGain_textfield = new JTextField();
  JTextField boost_PhiGain_textfield = new JTextField();
  JTextField boost_PhiPower_textfield = new JTextField();
...
//    JCheckBox eventDensityCheckbox = new JCheckBox("Density");
    ...
    private int _InitActiveDataSourceNum = 0;
...
//	        eventDensityCheckbox.addItemListener(this);
        ...
        // change a place, they are OK now. weird.
        ButtonGroup group = new ButtonGroup();
 //       group.add(boost1Radio);
//        group.add(selfBoost2Radio);
        group.add(powerBoostRadio);
        group.add(neighborBoostRadio);
        group.add(phiBoostRadio);
        group.add(noBoostRadio);
        group.add(randomBoostRadio);
        group.add(simlatedAnnealingBoostRadio);
//        group.add(powerneighborBoostRadio);
//        group.add(phineighborBoostRadio);
        
        
        ///////////
       
        northPanel.add(this.noBoostRadio );
        noBoostRadio .addActionListener(this);

       
        northPanel.add(this.powerBoostRadio );
        powerBoostRadio .addActionListener(this);
//        
        northPanel.add(this.neighborBoostRadio );
        neighborBoostRadio .addActionListener(this);
        
        northPanel.add(this.phiBoostRadio );
        phiBoostRadio .addActionListener(this);

//        northPanel.add(this.powerneighborBoostRadio );
//        powerneighborBoostRadio .addActionListener(this);
//        
//        northPanel.add(this.phineighborBoostRadio );
//        phineighborBoostRadio .addActionListener(this);
        northPanel.add(this.randomBoostRadio );
        randomBoostRadio .addActionListener(this);
        

        northPanel.add(this.simlatedAnnealingBoostRadio);
        simlatedAnnealingBoostRadio.addActionListener(this);
...
//        rightPanel.add("North", scrollPane);
        ...
        for (int i=0; i<24; i++)
        {
      	  nodestatus.add(new JCheckBox(Integer.toString(i)));
        }

        
        
     // insertVerticalSeparator(rightPanel);
     // rightPanel.add(nodegroup);
        rightPanel.add(allnode);
        
        for (int i=0; i<24; i++)
        {
        	rightPanel.add(nodestatus.get(i));
        }

        
        allnode.addItemListener(this);
        
        for (int i=0; i<24; i++)
        {
        	nodestatus.get(i).addItemListener(this);
        }

        
//        insertVerticalSeparator(rightPanel);
//        insertSeparator(rightPanel);
        
        rightPanel.add(new JLabel("Boost Duration"));
        rightPanel.add(boost_duration_textfield);
        boost_duration_textfield.setText(Double.toString(sim.robotList.get(0).boostDuration)+" ");
        boost_duration_textfield.addActionListener(this);
        
        
        rightPanel.add(new JLabel("Boost Iteration"));
        rightPanel.add(boost_iteration_textfield);
        boost_iteration_textfield.setText(Double.toString(sim.robotList.get(0).iteration));
        boost_iteration_textfield.addActionListener(this);
        
        
        rightPanel.add(new JLabel("Power Gain nu"));
        rightPanel.add(boost_PowerGain_textfield);
        boost_PowerGain_textfield.setText(Double.toString(sim.robotList.get(0).PowerGain)+" ");
        boost_PowerGain_textfield.addActionListener(this);
       
        rightPanel.add(new JLabel("Power lamda"));
        rightPanel.add(boost_Power_textfield);
        boost_Power_textfield.setText(Double.toString(sim.robotList.get(0).Power)+" ");
        boost_Power_textfield.addActionListener(this);
        
        rightPanel.add(new JLabel("Neighbor Gain g"));
        rightPanel.add(boost_neighborgain_textfield);
        boost_neighborgain_textfield.setText(Double.toString(sim.robotList.get(0).neighborGain)+" ");
        boost_neighborgain_textfield.addActionListener(this);
        
        rightPanel.add(new JLabel("Neighbor power "));
        rightPanel.add(boost_neighborpower_textfield);
        boost_neighborpower_textfield.setText(Double.toString(sim.robotList.get(0).neighborPower)+" ");
        boost_neighborpower_textfield.addActionListener(this);
        
        rightPanel.add(new JLabel("Phi Gain eta"));
        rightPanel.add(boost_PhiGain_textfield);
        boost_PhiGain_textfield.setText(Double.toString(sim.robotList.get(0).PhiGain)+" ");
        boost_PhiGain_textfield.addActionListener(this);
        
        rightPanel.add(new JLabel("Phi Power kar"));
        rightPanel.add(boost_PhiPower_textfield);
        boost_PhiPower_textfield.setText(Double.toString(sim.robotList.get(0).PhiPower)+" ");
        boost_PhiPower_textfield.addActionListener(this);
        
        updateObstaclesButton.setPreferredSize(new Dimension(200, 40));
        updateObstaclesButton.setMaximumSize(new Dimension(160, 80));
...
//                    RobotTable.setValueAt(sim.robotList.get(i5).heading, i5, 3);
//                }
//            }      
//    }
...
//        		this.updateTable();
        		for(int i=0; i< sim.robotList.size(); i++)
        		{
        			sim.robotList.get(i).indexNum = 1;
        		}
        	}
        }
        ...
        if(src == noBoostRadio)
        {
        	  for (int i1 = 0; i1 < sim.robotList.size(); i1++)
              {
              	 sim.robotList.get(i1).smallDetectionBoost = 10;
                   sim.robotList.get(i1).boosttype = 0;                                     
                   sim.robotList.get(i1).indexNum=1;
...
//                	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
                     sim.robotList.get(i1).boosttype = 2;
                     sim.robotList.get(i1).iteration=1;
                     sim.robotList.get(i1).indexNum=1;
                }

            }
         if (src == neighborBoostRadio )
         {
                 ...
                 for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                 {
                 	 sim.robotList.get(i1).smallDetectionBoost = 10;
//                 	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
                      sim.robotList.get(i1).boosttype = 3;
                      sim.robotList.get(i1).iteration=1;
                      sim.robotList.get(i1).indexNum=1;
                 }
         ...
         if (src == phiBoostRadio )
         {
                 ...
                 for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                 {
                 	 sim.robotList.get(i1).smallDetectionBoost = 10;
//                 	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
                      sim.robotList.get(i1).boosttype = 4;
                      sim.robotList.get(i1).iteration=1;
                      sim.robotList.get(i1).indexNum=1;
                 }

             }
         if (src == randomBoostRadio)
         {
        	 for (int i1 =0; i1< sim.robotList.size(); i1++)
        	 {
              	 sim.robotList.get(i1).smallDetectionBoost = 10;
//             	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
                  sim.robotList.get(i1).boosttype = 10;
               
                  sim.robotList.get(i1).iteration=1;
                  sim.robotList.get(i1).indexNum=1;
        	 }
         }
         if (src == simlatedAnnealingBoostRadio)
         {
        	 for (int i1 =0; i1< sim.robotList.size(); i1++)
        	 {
              	 sim.robotList.get(i1).smallDetectionBoost = 10;
//             	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
                  sim.robotList.get(i1).boosttype = 20;
...
//         if (src == powerneighborBoostRadio )
//         {
//
//                 for (int i1 = 0; i1 < sim.robotList.size(); i1++)
//                 {
//                 	 sim.robotList.get(i1).smallDetectionBoost = 10;
////                 	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
//                      sim.robotList.get(i1).boosttype = 5;
//                      sim.robotList.get(i1).iteration=1;
//                      sim.robotList.get(i1).indexNum=1;
//                 }
//         }
//
//                 if (src == phineighborBoostRadio )
//                 {
//
//                         for (int i1 = 0; i1 < sim.robotList.size(); i1++)
//                         {
//                         	 sim.robotList.get(i1).smallDetectionBoost = 10;
////                         	 sim.robotList.get(i1).ultraLowDetectionBoost = 1000000;
//                              sim.robotList.get(i1).boosttype = 6;
//                              sim.robotList.get(i1).iteration=1;
//                              sim.robotList.get(i1).indexNum=1;
//                         }
//                 }
        ...
        if (src == boost_PhiGain_textfield)
        {
            double phigain;
            try
            {
                phigain = Double.valueOf(boost_PhiGain_textfield.getText());
            } catch (NumberFormatException ecp)
            {
                phigain = -1;
            }

            if (phigain >= 0 && phigain < 20000)
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).PhiGain = phigain;
                }
            }
            else
            {
            	boost_PhiGain_textfield.setText("[0-19999)");
            }
        }
        
        if (src == boost_PhiPower_textfield)
        {
            double phipower;
            try
            {
                phipower = Double.valueOf(boost_PhiPower_textfield.getText());
            } catch (NumberFormatException ecp)
            {
                phipower = -1;
            }

            if (phipower >= 0 && phipower < 100)
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).PhiPower = phipower;
                }
            }
            else
            {
            	boost_PhiPower_textfield.setText("[0-100)");
            }
        }
        if(src ==boost_Power_textfield)
        {
        	double power;
        	try
        	{
        		power = Double.valueOf(boost_Power_textfield.getText());
        	} catch(NumberFormatException ecp)
        	{
        		power = -1;
        	}
        	if (power >= 0 )
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).Power = power;
                }
            }
            else
            {
            	boost_Power_textfield.setText("[0-)");
            }

        }

        if(src ==boost_PowerGain_textfield)
        {
        	double powerGain;
        	try
        	{
        		powerGain = Double.valueOf(boost_PowerGain_textfield.getText());
        	} catch(NumberFormatException ecp)
        	{
        		powerGain = -1;
        	}
        	if (powerGain >= 0 )
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).PowerGain = powerGain;
                }
            }
            else
            {
            	boost_PowerGain_textfield.setText("[0-)");
            }

        }

        if(src ==boost_duration_textfield)
        {
        	double duration;
        	try
        	{
        		duration = Double.valueOf(boost_duration_textfield.getText());
        	} catch(NumberFormatException ecp)
        	{
        		duration = -1;
        	}
        	if (duration >= 0 && duration < 2000)
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).boostDuration = duration;
                }
            }
            else
            {
            	boost_duration_textfield.setText("[0-2000)");
            }

        }

        if (src == boost_neighborgain_textfield)
        {
            double neighborgain;
            try
            {
                neighborgain = Double.valueOf(boost_neighborgain_textfield.getText());
            } catch (NumberFormatException ecp)
            {
                neighborgain = -1;
            }

            if (neighborgain >= 0 && neighborgain < 20000)
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).neighborGain = neighborgain;
                }
            }
            else
            {
            	boost_neighborgain_textfield.setText("[0-19999)");
            }
        }
        if (src == boost_neighborpower_textfield)
        {
            double neighborpower;
            try
            {
                neighborpower = Double.valueOf(boost_neighborpower_textfield.getText());
            } catch (NumberFormatException ecp)
            {
                neighborpower = -1;
            }

            if (neighborpower >= 0 && neighborpower < 10)
            {
                for (int i1 = 0; i1 < sim.robotList.size(); i1++)
                {
                    sim.robotList.get(i1).neighborPower = neighborpower;
                }
            }
            else
            {
            	boost_neighborpower_textfield.setText("[0-10)");
            }
        }
...
//        }
    ...
    public synchronized void itemStateChanged(ItemEvent e)
    {
       ...
       if (src == realTimeObjFuncValueCheckbox)
        ...
        {
//        	System.out.println("allnode boost"+ on);
        	   if(on == true)
//        		sim.robotList.get(i1).Node_on = true;
	        	   {	
        		   
        		   
        		   		for (int i=0; i<24;i++)
        		   		{
        		   			if(sim.realTimeRobotNumber > i)
        		   			{
        		   				if(!nodestatus.get(i).isSelected())
        		   				{
        		   					nodestatus.get(i).doClick();
        		   					sim.robotList.get(i).Node_on = true;
        		   				}
        		   			}
        		   		}
	        	   }
		    	  ...
		    	  {	  for (int i=0; i<24;i++)
  		   				{
		  		   			if(sim.realTimeRobotNumber > i)
		  		   			{
		  		   				...
		  		   				{
		  		   					nodestatus.get(i).doClick();
		  		   					sim.robotList.get(i).Node_on = false;
		  		   				}
		  		   			}
  		   				}
		    	  }
